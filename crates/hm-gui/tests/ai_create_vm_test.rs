...
use hm_gui::{
    AutomationHandle, CommandResult, DialogType, FormType, GuiCommand,
    NavigateParams, SelectVmParams, SelectionMode, ViewType, VmActionType,
    get_gui_tools, get_openai_tools,
};
...
fn test_ai_creates_vm_through_gui() {
            ...
            if let Some(request) = receiver.try_recv() {
                ...
                let result = match &request.command {
                    GuiCommand::OpenDialog(DialogType::CreateVm) => {
                        CommandResult::success("gui.dialog.open", Some(serde_json::json!({
                            "dialog": "create_vm",
                            "status": "opened"
                        })))
                    }
                    GuiCommand::SetFormField(params) => {
                        CommandResult::success("gui.form.set_field", Some(serde_json::json!({
                            "form": params.form,
                            "field": &params.field,
                            "value": &params.value,
                            "status": "set"
                    ...
                    GuiCommand::SubmitDialog(DialogType::CreateVm) => {
                        CommandResult::success("gui.dialog.submit", Some(serde_json::json!({
                            "dialog": "create_vm",
                            "status": "submitted",
                            "vm_id": "vm-001",
                            "vm_name": "ai-test-vm"
                    ...
                    GuiCommand::GetState => {
                        CommandResult::success("gui.get_state", Some(serde_json::json!({
                    ...
                    GuiCommand::VmAction(VmActionType::Start) => {
                        CommandResult::success("gui.vm.action", Some(serde_json::json!({
                            "action": "start",
                            "vm_id": "vm-001",
                            "status": "starting"
                        })))
    ...
    let result = handle.set_field(FormType::CreateVm, "name", "ai-test-vm").unwrap();
    assert!(result.success);
    ...
    let result = handle.set_field(FormType::CreateVm, "memory_mb", 8192).unwrap();
    assert!(result.success);
    println!("   ✅ Memory set to 8192 MB");

    let result = handle.set_field(FormType::CreateVm, "network_enabled", true).unwrap();
    assert!(result.success);
    println!("   ✅ Network enabled");

    // 3c: Submit the form
    println!("\n   Step 3: Submitting VM creation...");
    let result = handle.execute(GuiCommand::SubmitDialog(DialogType::CreateVm)).unwrap();
    assert!(result.success);
    ...
    println!("\n🎉 Success! AI agent created VM through {} GUI commands", commands_processed);
}
...
fn test_ai_vm_creation_via_json() {
    ...
    for (i, json) in commands.iter().enumerate() {
        ...
        match cmd {
            Ok(parsed) => {
                println!("   Command {}: ✅ Valid - {:?}", i + 1, std::mem::discriminant(&parsed));
            }
            Err(e) => {
