...
pub fn toolbar(ui: &mut Ui, state: &mut AppState) -> ToolbarAction {
    ...
    egui::menu::bar(ui, |ui| {
        ...
        ui.separator();
        if ui.add(primary_button(&format!("{} New VM", icons::ADD))).clicked() { state.show_create_dialog = true; }
        ui.separator();
        if ui.button(format!("{} Refresh", icons::REFRESH)).clicked() { action = ToolbarAction::Refresh; }
        ui.checkbox(&mut state.auto_refresh, "Auto");
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if ui.button(icons::SETTINGS).clicked() { state.show_settings = true; }
            if ui.button(icons::INFO).clicked() { state.show_about = true; }
            ui.separator();
...
pub enum ToolbarAction { None, Refresh }
...
pub fn vm_list_sidebar(ui: &mut Ui, state: &mut AppState) -> SidebarAction {
    ...
    ui.vertical(|ui| {
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| { stats_bar(ui, &state.vm_counts()); });
        });
        ui.add_space(8.0);
        ui.separator();
        ui.add_space(4.0);
        ...
        egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
            if state.vms.is_empty() {
                ui.vertical_centered(|ui| {
                    ...
                    ui.add_space(16.0);
                    if ui.add(primary_button("Create your first VM")).clicked() { state.show_create_dialog = true; }
                });
            } else {
                ...
                for vm in vms {
                    ...
                    match vm_action {
                        VmAction::Select => { action = SidebarAction::SelectVm(vm.id.clone()); }
                        VmAction::Start => { action = SidebarAction::StartVm(vm.id.clone()); }
                        VmAction::Stop => { action = SidebarAction::StopVm(vm.id.clone()); }
                        VmAction::Pause => { action = SidebarAction::PauseVm(vm.id.clone()); }
                        VmAction::OpenConsole => { action = SidebarAction::OpenConsole(vm.id.clone()); }
                        _ => {}
                    }
                }
            }
...
pub fn main_content(ui: &mut Ui, state: &mut AppState) -> ContentAction {
    ...
    if let Some(console_vm_id) = &state.console_vm.clone() {
        if let Some(vm) = state.vms.get(console_vm_id) { action = console_view(ui, vm); }
    ...
    ui.vertical_centered(|ui| {
        ...
        if !state.vms.is_empty() {
            ...
            ui.horizontal(|ui| {
                ...
                stat_card(ui, "Stopped", &counts.stopped.to_string(), colors.text_secondary);
            });
            ui.add_space(24.0);
            ui.label(RichText::new("Select a VM from the list to view details").color(colors.text_secondary));
        } else {
            ui.label(RichText::new("No VMs yet. Create one to get started!").color(colors.text_secondary));
            ui.add_space(24.0);
            egui::Frame::new().fill(colors.surface).inner_margin(24.0).corner_radius(CornerRadius::same(8)).show(ui, |ui| {
...
fn stat_card(ui: &mut Ui, label: &str, value: &str, color: Color32) {
    let colors = AppColors::default();
    egui::Frame::new().fill(colors.surface).inner_margin(16.0).corner_radius(CornerRadius::same(8)).show(ui, |ui| {
        ui.vertical_centered(|ui| {
    ...
    egui::Frame::new().fill(Color32::BLACK).show(ui, |ui| {
        ...
        if vm.state != VmStateApi::Running {
            ui.vertical_centered(|ui| {
                ...
                ui.label(RichText::new("VM is not running").color(colors.text_secondary).size(20.0));
                ui.label(RichText::new("Start the VM to see the console").color(colors.text_secondary));
            });
...
pub fn settings_dialog(ui: &mut Ui, state: &mut AppState) -> bool {
    ...
    egui::Grid::new("settings_grid").num_columns(2).spacing([10.0, 10.0]).show(ui, |ui| {
        ui.label("Backend URL:"); ui.text_edit_singleline(&mut state.backend_url); ui.end_row();
        ui.label("Auto-refresh:"); ui.checkbox(&mut state.auto_refresh, "Enabled"); ui.end_row();
        ui.label("Refresh interval:"); ui.add(egui::DragValue::new(&mut state.refresh_interval).range(1..=60).suffix("s")); ui.end_row();
    });
    ui.add_space(16.0);
    ui.horizontal(|ui| {
        if ui.button("Save").clicked() { open = false; }
...
pub fn about_dialog(ui: &mut Ui) -> bool {
    ...
    ui.vertical_centered(|ui| {
        ...
        ui.label(RichText::new(format!("Version {}", env!("CARGO_PKG_VERSION"))).color(colors.text_secondary));
        ui.add_space(16.0);
