...
async fn test_io_exit_to_serial_device() {
    let (manager, serial, _timer) = setup_device_manager();

    // Test I/O OUT to serial port (write)
    let port = 0x3F8; // THR register
    let data: u32 = 0x48; // 'H'

    if let Some(handle) = manager.find_io_device(port) {
...
async fn test_io_exit_to_timer_device() {
    let (manager, _serial, _timer) = setup_device_manager();
...
async fn test_io_exit_unmapped_port() {
    ...
    assert!(manager.find_io_device(port).is_none());
}
...
async fn test_mmio_exit_to_device() {
    ...
    manager
        .register_device("mmio_serial".to_string(), mmio_serial.clone())
        .unwrap();
    manager
        .register_mmio_region("mmio_serial".to_string(), 0x1000_0000, 0x1000)
        .unwrap();
    mmio_serial.write().init().await.unwrap();

    // Test MMIO write
    let addr = 0x1000_0000;
    if let Some(handle) = manager.find_mmio_device(addr) {
        ...
        // Verify write
        let output = mmio_serial.read().output_string();
...
async fn test_mmio_exit_read_write() {
    ...
    if let Some(handle) = manager.find_mmio_device(0x2000_0000) {
        // Write to MMIO
...
async fn test_mmio_exit_unmapped_address() {
    ...
    assert!(manager.find_mmio_device(addr).is_none());
}
...
async fn test_vm_exit_routing_serial() {
    ...
    assert!(serial.read().output_string().contains('H'));
}

#[tokio::test]
async fn test_vm_exit_routing_timer() {
    let (manager, _serial, timer) = setup_device_manager();
