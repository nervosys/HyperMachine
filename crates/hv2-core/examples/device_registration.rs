...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager.register_device("serial".to_string(), serial.clone())?;
    ...
    manager.register_device("timer".to_string(), timer.clone())?;
    ...
    if let Some(handle) = manager.find_io_device(0x3F8) {
    ...
    if let Some(handle) = manager.find_io_device(0x40) {
    ...
    if manager.find_io_device(0x1000).is_none() {
    ...
    manager.register_device("serial2".to_string(), serial2.clone())?;

    match manager.register_io_port_range("serial2".to_string(), 0x3F0, 0x3FF) {
        Ok(_) => println!("  ❌ Overlap detection failed!"),
        Err(e) => println!("  ✓ Overlap correctly detected: {}", e),
    }

    // Register non-overlapping range for COM2
    manager.register_io_port_range("serial2".to_string(), 0x2F8, 0x2FF)?;
    println!("  ✓ COM2 registered at non-overlapping range: 0x2F8-0x2FF");

    // Show all registered devices
    println!("\n📋 Registered Devices:");
    if let Some(device) = manager.get_device("serial") {
        println!(
            "  • serial: type={:?}, name='{}'",
            device.read().device_type(),
            device.read().name()
        );
    }
    if let Some(device) = manager.get_device("timer") {
        println!(
            "  • timer: type={:?}, name='{}'",
            device.read().device_type(),
            device.read().name()
    ...
    if let Some(device) = manager.get_device("serial2") {
        println!(
            "  • serial2: type={:?}, name='{}'",
            device.read().device_type(),
            device.read().name()
    ...
    manager.register_device("mmio_serial".to_string(), mmio_device.clone())?;
    println!("  ✓ MMIO device registered");

    // Register MMIO region (1 MB at 0x1000_0000)
    manager.register_mmio_region("mmio_serial".to_string(), 0x1000_0000, 0x10_0000)?;
    println!("  ✓ MMIO region registered: 0x1000_0000-0x1010_0000 (1 MB)");

    // Test MMIO lookup
    if let Some(handle) = manager.find_mmio_device(0x1000_5000) {
        println!(
    ...
    let mmio_device2 = Arc::new(RwLock::new(SerialDevice::new(
        "MMIO_SERIAL2".to_string(),
        0x2000_0000,
    )));
    manager.register_device("mmio_serial2".to_string(), mmio_device2.clone())?;

    match manager.register_mmio_region("mmio_serial2".to_string(), 0x1000_8000, 0x10_0000) {
    ...
    manager.register_mmio_region("mmio_serial2".to_string(), 0x2000_0000, 0x10_0000)?;
