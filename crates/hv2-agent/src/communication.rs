...
impl MessagePayload {
    ...
    pub fn json<T: Serialize>(value: &T) -> CommResult<Self> {
        let data =
            serde_json::to_vec(value).map_err(|e| CommError::InvalidMessage(e.to_string()))?;
        Ok(Self {
    ...
    pub fn as_json<T: for<'de> Deserialize<'de>>(&self) -> CommResult<T> {
        serde_json::from_slice(&self.data).map_err(|e| CommError::InvalidMessage(e.to_string()))
    }

    /// Get as text
    pub fn as_text(&self) -> CommResult<String> {
        String::from_utf8(self.data.clone()).map_err(|e| CommError::InvalidMessage(e.to_string()))
    }
...
impl MessageRouter {
    ...
    pub fn register_agent(&mut self, agent: AgentInfo) {
        ...
        self.queues
            .insert(agent_id, MessageQueue::new(self.default_queue_size));
    }
...
mod tests {
    ...
    fn test_message_queue_push_pop() {
        ...
        queue
            .push(Message::new("a", "b", MessagePayload::text("1")))
            .unwrap();
        queue
            .push(Message::new("a", "b", MessagePayload::text("2")))
            .unwrap();
    ...
    fn test_message_queue_expired_cleanup() {
        ...
        queue
            ...
            .unwrap();
        queue
            .push(Message::new("a", "b", MessagePayload::text("stays")))
            .unwrap();
    ...
    fn test_channel_history() {
        let mut channel = Channel::new("test-channel").with_max_history(3);

        channel.add_to_history(Message::broadcast(
            "a",
            "test-channel",
            MessagePayload::text("1"),
        ));
        channel.add_to_history(Message::broadcast(
            "a",
            "test-channel",
            MessagePayload::text("2"),
        ));
        channel.add_to_history(Message::broadcast(
            "a",
            "test-channel",
            MessagePayload::text("3"),
        ));
        channel.add_to_history(Message::broadcast(
            "a",
            "test-channel",
            MessagePayload::text("4"),
        ));
    ...
    fn test_router_find_by_capability() {
        ...
        router.register_agent(AgentInfo::new("agent-1", "Agent One").with_capability("monitoring"));
        router.register_agent(
            AgentInfo::new("agent-2", "Agent Two")
                .with_capability("monitoring")
                .with_capability("alerting"),
        );
        router.register_agent(AgentInfo::new("agent-3", "Agent Three").with_capability("alerting"));
